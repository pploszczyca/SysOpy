...
void addFilesToArray(FILE *firstFile, FILE *secondFile, struct rowMergedFile **arrayOfFiles, int index){
    ...
    arrayOfFiles[index]->n_rows = getFileLines(firstFile)*2;
...
void readFromTemporaryFileToArray(FILE* temp_file, struct rowMergedFile **arrayOfFiles, int index){
    ...
    arrayOfFiles[index]->n_rows = getFileLines(temp_file);

    arrayOfFiles[index]->rows = (char **)calloc(arrayOfFiles[index]->n_rows, sizeof(char *));

    while ((read = getline(&line, &len, temp_file)) != -1) {
        arrayOfFiles[index]->rows[i] = (char *)calloc(len, sizeof(char));
...
char **splitTwoFileNames(char *notSplitFileNames){
    char **tmpNotSplitFileNames = (char **)calloc(1, sizeof(char *));
    tmpNotSplitFileNames[0] = calloc(sizeof(notSplitFileNames)/sizeof(notSplitFileNames[0]), sizeof(char));
    strcpy(tmpNotSplitFileNames[0], notSplitFileNames);
    char **result = splitFileNames(tmpNotSplitFileNames, 1);
    free(tmpNotSplitFileNames[0]);
    free(tmpNotSplitFileNames);
    return result;
}